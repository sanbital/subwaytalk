...
- 방 키는 `노선|all` — **노선 하나당 방 하나**입니다(역마다도, 방향으로도 갈라지지 않음).

  원래는 `노선|방향` 이었습니다. 그런데 지하 구간에서는 저정밀 측위만 잡히고(실측 ±2000m)
  오차가 역 간격보다 커서 진행 방향 표가 모이지 않습니다. 방향이 없으면 방이 없고, 방이 없으면
  입력창이 잠깁니다 — 메시징 서비스가 위치 판정 실패로 주행 내내 멈추는 셈이었습니다.
  방향은 방을 가르는 기준에서 빼고 헤더 표시용으로만 남겼습니다. 상·하행이 한 방에 섞이는 건
  감수한 비용이고, 유저풀이 두 방을 채울 만큼 커지면 다시 나눕니다.
- **대화가 없을 때는 빈 화면 대신 첫 수(opening move)를 보여줍니다.** 여기가 뭐 하는 곳인지 한 줄,
  오늘의 질문(누르면 투표하면서 그 답이 첫마디로 올라감), 한 번 누르면 그대로 전송되는 말문 트기 버튼.
  누군가 말을 시작하면 전부 사라집니다. 질문·게임은 별도 기능이 아니라 대화의 마중물이라는 뜻입니다.
- 헤더의 참여자 수는 이 방에서 최근 15분간 말한 사람 수입니다(AI 동행 포함).
  예전에는 앱 전역 KV 프레즌스를 세서 노선과 무관했고 사실상 늘 `1` 이었습니다.
